    ...
    def __call__(self, rlds_batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if rlds_batch["observation"]["image_primary"].shape[0] == 1:
            ...
            if self.use_wrist_image:
                assert rlds_batch["observation"]["image_primary"].shape[0] == rlds_batch["observation"]["image_wrist"].shape[0]
                img = [img, Image.fromarray(rlds_batch["observation"]["image_wrist"][0])]
        else:
            img = [Image.fromarray(ix) for ix in rlds_batch["observation"]["image_primary"]]
            if self.use_wrist_image:
                assert rlds_batch["observation"]["image_primary"].shape[0] == rlds_batch["observation"]["image_wrist"].shape[0]
                wrist_img = [Image.fromarray(ix) for ix in rlds_batch["observation"]["image_wrist"]]
                img = [[a, b] for a, b in zip(img, wrist_img)]
        ...
        assert self.action_tokenizer is None
        ...
        if isinstance(img, list):
            pixel_values = [self.image_transform(ix) for ix in img]
        else:
        ...
        if isinstance(self.base_tokenizer, Qwen2TokenizerFast):
            ...
            if self.action_tokenizer is None:
                ...
                labels[: -(len(action) + 2)] = IGNORE_INDEX
        elif isinstance(self.base_tokenizer, LlamaTokenizerFast):
            if self.action_tokenizer is None:
                labels[: -1] = IGNORE_INDEX
            else:
            ...
            raise ValueError(f"Tokenizer of type `{type(self.base_tokenizer)}` is not explicitly handled!")
